...
const Register = () => {
 const {regData, handelregChange, handelregClick, showreg, setShowreg} = useContext(userContext)

  return (
    <section className="min-h-screen bg-[#09090B] flex items-center justify-center p-5">
      ...
      <div className="relative w-full max-w-md bg-[#111827]/80 backdrop-blur-xl border border-gray-700 rounded-3xl p-8 shadow-[0_0_50px_rgba(0,0,0,.5)]">
        ...
        <p className="text-gray-400 text-center mt-3 mb-2">
      ...
      <input
        ...
        <div className="relative">
          ...
          >
            {showreg ? <EyeOff size={20} /> : <Eye size={20} />}
          </button>

        </div>

     
        <button onClick={handelregClick} className="cursor-pointer w-full mt-6 bg-gradient-to-r from-violet-600 to-cyan-500 py-3 rounded-xl text-white font-semibold hover:scale-[1.02] duration-300">
          Register
        </button>

        <p className="text-center text-gray-400 mt-6">
          I have an account?
          <Link to="/" className="cursor-pointer text-violet-400 cursor-pointer ml-2 text-blue-600">
            Sign In
          </Link>
        </p>
